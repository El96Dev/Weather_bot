...
def get_coordinates_from_city_button(
    cities: dict, button_city: str, button_country: str, button_state: str
) -> dict | None:
    coordinates = dict()
    for city in cities:
        if (
            city["name"] == button_city
            and city["country"] == button_country
            and city["state"] == button_state
        ):
...
def create_current_weather_report(city: str, response: dict) -> str:
    ...
    report += (
        "Ощущается как: "
        + str(round(response["main"]["feels_like"] - 273.15, 2))
        + "\n"
    )
...
def create_air_polution_report(city: str, response: dict) -> str:
    ...
    report += (
        "Концентрация CO: " + str(response["list"][0]["components"]["co"]) + " мкг/м3\n"
    )
    report += (
        "Концентрация NO: " + str(response["list"][0]["components"]["no"]) + " мкг/м3\n"
    )
    report += (
        "Концентрация NO\u2082: "
        + str(response["list"][0]["components"]["no2"])
        + " мкг/м3\n"
    )
    report += (
        "Концентрация O3: " + str(response["list"][0]["components"]["o3"]) + " мкг/м3\n"
    )
    report += (
        "Концентрация SO\u2082: "
        + str(response["list"][0]["components"]["so2"])
        + " мкг/м3\n"
    )
    report += (
        "Концентрация PM\u2082.\u2085: "
        + str(response["list"][0]["components"]["pm2_5"])
        + " мкг/м3\n"
    )
    report += (
        "Концентрация PM\u2081\u2080: "
        + str(response["list"][0]["components"]["pm10"])
        + " мкг/м3\n"
    )
    report += (
        "Концентрация NH\u2083: "
        + str(response["list"][0]["components"]["nh3"])
        + " мкг/м3\n"
    )
    return report
...
def create_weather_forecast(city: str, response: dict) -> list[str]:
    ...
    for forecast in forecasts:
        ...
        report += (
            "Температура: " + str(round(forecast["main"]["temp"] - 273.15, 2)) + "\n"
        )
        report += (
            "Ощущается как: "
            + str(round(forecast["main"]["feels_like"] - 273.15, 2))
            + "\n"
        )
